...
describe('Multilingual Edge Cases', () => {
  describe('Russian complex plurals', () => {
    it('should handle 11-14 (many)', () => {
      const msg = '{n, plural, one {# день} few {# дня} many {# дней} other {# дней}}';
  ...
  describe('Arabic 6-form plurals', () => {
    it('should handle all 6 forms', () => {
      const msg = '{n, plural, zero {صفر} one {واحد} two {اثنان} few {# قليل} many {# كثير} other {# آخر}}';
  ...
  describe('Polish edge cases', () => {
    it('should handle 12-14 (many)', () => {
      const msg = '{n, plural, one {# przedmiot} few {# przedmioty} many {# przedmiotów} other {# przedmiotów}}';
  ...
  describe('Mixed scripts', () => {
    it('should handle Latin + Cyrillic', () => {
      const msg = 'User {name} имеет {n, plural, one {# файл} other {# файлов}}';
      expect(formatICUMessage(msg, { name: 'Alice', n: 5 }, 'ru')).toBe('User Alice имеет 5 файлов');
    });

    it('should handle Latin + Arabic', () => {
      const msg = 'User {name} لديه {n, plural, one {ملف} other {# ملفات}}';
      expect(formatICUMessage(msg, { name: 'Bob', n: 3 }, 'ar')).toBe('User Bob لديه 3 ملفات');
    });

    it('should handle Latin + CJK', () => {
      const msg = 'User {name}は{n}個のアイテムを持っています';
      expect(formatICUMessage(msg, { name: 'Charlie', n: 5 })).toBe('User Charlieは5個のアイテムを持っています');
    });
  });
