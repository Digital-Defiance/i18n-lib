...
describe('ICU MessageFormat Specification Compliance', () => {
  describe('Official ICU Examples from Unicode.org', () => {
    it('Example 1: Simple argument substitution', () => {
      ...
      const ast = parse('At {time, time} on {date, date}, there was {event} on planet {planet, number, integer}.');
      expect(ast.elements).toHaveLength(9);
      expect(ast.elements[0]).toEqual({ type: NodeType.LITERAL, value: 'At ' });
      expect(ast.elements[1]).toEqual({ type: NodeType.ARGUMENT, name: 'time', format: 'time' });
      expect(ast.elements[2]).toEqual({ type: NodeType.LITERAL, value: ' on ' });
      expect(ast.elements[3]).toEqual({ type: NodeType.ARGUMENT, name: 'date', format: 'date' });
      expect(ast.elements[4]).toEqual({ type: NodeType.LITERAL, value: ', there was ' });
      expect(ast.elements[5]).toEqual({ type: NodeType.ARGUMENT, name: 'event' });
      expect(ast.elements[6]).toEqual({ type: NodeType.LITERAL, value: ' on planet ' });
      expect(ast.elements[7]).toEqual({ type: NodeType.ARGUMENT, name: 'planet', format: 'number', style: 'integer' });
    ...
    it('Example 2: Plural with exact matches (simplified)', () => {
      ...
      const ast = parse('{count, plural, zero {no results} one {one result} other {# results}}');
    ...
    it('Example 3: Select for gender', () => {
      // From ICU spec
      const ast = parse('{gender, select, male {He} female {She} other {They}} liked this.');
    ...
    it('Example 4: SelectOrdinal for rankings', () => {
      // From ICU spec
      const ast = parse('You finished {place, selectordinal, one {#st} two {#nd} few {#rd} other {#th}}!');
    ...
    it('Example 5: Nested plural in select', () => {
      ...
      const ast = parse('{gender, select, male {{count, plural, one {He has one item} other {He has # items}}} female {{count, plural, one {She has one item} other {She has # items}}}}');
      const select = ast.elements[0] as any;
      expect(select.type).toBe(NodeType.SELECT);
  ...
  describe('FormatJS Examples (React Intl standard)', () => {
    it('FormatJS Example 1: Basic plural', () => {
      const ast = parse('Cart: {itemCount, plural, one {# item} other {# items}}');
      expect(ast.elements).toHaveLength(2);
    ...
    it('FormatJS Example 2: Rich text with select', () => {
      const ast = parse('{name} {action, select, like {likes} other {commented on}} your photo.');
    ...
    it('FormatJS Example 3: Date and time formatting', () => {
      const ast = parse('Sale begins {start, date, long} at {start, time, short}');
      expect(ast.elements).toHaveLength(4);
      expect(ast.elements[1]).toEqual({ type: NodeType.ARGUMENT, name: 'start', format: 'date', style: 'long' });
      expect(ast.elements[3]).toEqual({ type: NodeType.ARGUMENT, name: 'start', format: 'time', style: 'short' });
    });
  ...
  describe('Edge Cases from ICU Specification', () => {
    ...
    it('Only text, no arguments', () => {
      ...
      expect(ast.elements[0]).toEqual({ type: NodeType.LITERAL, value: 'This is just plain text.' });
    });
    ...
    it('Hash placeholder in plural', () => {
      ...
      expect(plural.cases.other.elements[0]).toEqual({ type: NodeType.LITERAL, value: '#' });
    });
    ...
    it('Escaped braces', () => {
      ...
      expect(ast.elements[2]).toEqual({ type: NodeType.LITERAL, value: "' and " });
      expect(ast.elements[3]).toEqual({ type: NodeType.LITERAL, value: '}' });
      expect(ast.elements[4]).toEqual({ type: NodeType.LITERAL, value: "' as delimiters" });
    });
  });
  ...
  describe('Complex Real-World Patterns', () => {
    it('E-commerce: Shopping cart with multiple variables', () => {
      const ast = parse('Your cart has {itemCount, plural, one {# item} other {# items}} totaling {total, number, currency}. {shipping, select, free {Free shipping!} standard {Standard shipping: {shippingCost, number, currency}} express {Express shipping: {shippingCost, number, currency}} other {Shipping calculated at checkout}}');
    ...
    it('Social: Notification with nested gender and plural', () => {
      const ast = parse('{senderGender, select, male {{count, plural, one {He sent you a message} other {He sent you # messages}}} female {{count, plural, one {She sent you a message} other {She sent you # messages}}} other {{count, plural, one {They sent you a message} other {They sent you # messages}}}}');
      const select = ast.elements[0] as any;
      expect(select.type).toBe(NodeType.SELECT);
    ...
    it('Calendar: Event with date, time, and plural attendees', () => {
      const ast = parse('{eventName} starts on {startDate, date, full} at {startTime, time, short}. {attendeeCount, plural, zero {No one is attending} one {# person is attending} other {# people are attending}}.{location, select, online {Join online at {url}} venue {Location: {venueName}} other {Location TBD}}');
  ...
  describe('Performance and Stress Tests', () => {
    it('Should handle deeply nested messages (4 levels)', () => {
      const ast = parse('{a, select, x {{b, select, y {{c, plural, one {{d, select, m {deep} other {nested}}} other {{d, select, m {deep} other {nested}}}}} other {{c, plural, one {text} other {text}}}}} other {text}}');
      expect(ast.elements[0].type).toBe(NodeType.SELECT);
    });

    it('Should handle many plural cases', () => {
      const ast = parse('{count, plural, zero {zero} one {one} two {two} few {few} many {many} other {other}}');
      const plural = ast.elements[0] as any;
      expect(Object.keys(plural.cases)).toHaveLength(6);
    });

    it('Should handle many select cases', () => {
      const ast = parse('{status, select, draft {Draft} pending {Pending} approved {Approved} rejected {Rejected} published {Published} archived {Archived} deleted {Deleted} other {Unknown}}');
      const select = ast.elements[0] as any;
  ...
  describe('Comparison with Industry Standards', () => {
    it('Should match React Intl behavior: plural with text', () => {
      ...
      const ast = parse('You have {unreadCount, plural, one {# unread message} other {# unread messages}}');
      const plural = ast.elements[1] as any;
      expect(plural.type).toBe(NodeType.PLURAL);
    ...
    it('Should match Vue I18n behavior: select with variables', () => {
      ...
      const ast = parse('{name} {gender, select, male {is online} female {is online} other {is online}}');
      expect(ast.elements).toHaveLength(3);
      expect(ast.elements[2].type).toBe(NodeType.SELECT);
    });
    ...
    it('Should match Angular i18n behavior: nested constructs', () => {
      ...
      const ast = parse('{minutes, plural, one {one minute ago} other {# minutes ago}}');
      const plural = ast.elements[0] as any;
