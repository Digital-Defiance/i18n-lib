...
describe('ICU MessageFormat Validator', () => {
  describe('Valid messages', () => {
    ...
    it('should validate nested structures', () => {
      const ast = parse('{gender, select, male {{count, plural, one {# item} other {# items}}} female {{count, plural, one {# item} other {# items}}} other {items}}');
      expect(() => validate(ast)).not.toThrow();
    });
  });
  ...
  describe('Plural validation', () => {
    ...
    it('should validate nested plurals', () => {
      const ast = parse('{outer, plural, one {{inner, plural, one {#} other {#}}} other {{inner, plural, one {#} other {#}}}}');
      expect(() => validate(ast)).not.toThrow();
    });
  });
  ...
  describe('Select validation', () => {
    ...
    it('should validate nested selects', () => {
      const ast = parse('{outer, select, a {{inner, select, x {text} other {text}}} other {{inner, select, y {text} other {text}}}}');
      expect(() => validate(ast)).not.toThrow();
    });
  });
  ...
  describe('SelectOrdinal validation', () => {
    ...
    it('should validate all CLDR ordinal forms', () => {
      const ast = parse('{place, selectordinal, zero {#th} one {#st} two {#nd} few {#rd} many {#th} other {#th}}');
      expect(() => validate(ast)).not.toThrow();
    });
    ...
    it('should reject invalid ordinal cases', () => {
      ...
      expect(() => validate(ast)).toThrow(ValidationError);
      expect(() => validate(ast)).toThrow("Invalid selectordinal case 'invalid'");
    });
  });

  describe('Nesting depth validation', () => {
    it('should allow reasonable nesting', () => {
      const ast = parse('{a, select, x {{b, select, y {{c, select, z {text} other {text}}} other {text}}} other {text}}');
      expect(() => validate(ast)).not.toThrow();
    });
    ...
    it('should allow custom max depth', () => {
      const ast = parse('{a, select, x {{b, select, y {text} other {text}}} other {text}}');
  ...
  describe('Complex real-world validation', () => {
    it('should validate e-commerce message', () => {
      const ast = parse('{gender, select, male {He} female {She} other {They}} ordered {itemCount, plural, one {# item} other {# items}} for {price, number, currency}');
      expect(() => validate(ast)).not.toThrow();
    });

    it('should validate notification message', () => {
      const ast = parse('{senderGender, select, male {He sent} female {She sent} other {They sent}} you {count, plural, one {a message} other {# messages}}');
      expect(() => validate(ast)).not.toThrow();
    });

    it('should validate calendar reminder', () => {
      const ast = parse('You have {count, plural, zero {no meetings} one {# meeting} other {# meetings}} on {date, date, long}');
      expect(() => validate(ast)).not.toThrow();
    });

    it('should validate social media post', () => {
      const ast = parse('{name} and {count, plural, zero {no one else} one {# other person} other {# other people}} liked this');
      expect(() => validate(ast)).not.toThrow();
    });
  });
  ...
  describe('Edge cases', () => {
    ...
    it('should validate deeply nested mixed constructs', () => {
      const ast = parse('{a, plural, one {{b, select, x {{c, plural, one {#} other {#}}} other {text}}} other {text}}');
      expect(() => validate(ast)).not.toThrow();
    });
  });
});
