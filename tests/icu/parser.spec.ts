...
describe('ICU MessageFormat Parser', () => {
  ...
  describe('SelectOrdinal messages', () => {
    ...
    it('should parse selectordinal with text', () => {
      const ast = parse(
        '{place, selectordinal, one {#st place} two {#nd place} other {#th place}}',
      );
      const ordinal = ast.elements[0] as any;
      expect(ordinal.cases.one.elements[1]).toEqual({
        type: NodeType.LITERAL,
        value: 'st place',
      });
    });
  ...
  describe('Nested messages', () => {
    ...
    it('should parse nested select in plural', () => {
      const ast = parse(
        '{count, plural, one {{gender, select, male {He} female {She}}} other {{gender, select, male {They} female {They}}}}',
      );
      const plural = ast.elements[0] as any;
  ...
  describe('Complex real-world messages', () => {
    ...
    it('should parse message with plural and text', () => {
      const ast = parse(
        'You have {count, plural, one {# message} other {# messages}} from {sender}',
      );
    ...
    it('should parse complex nested message', () => {
      const ast = parse(
        '{gender, select, male {He sent you {count, plural, one {# message} other {# messages}}} female {She sent you {count, plural, one {# message} other {# messages}}}}',
      );
      expect(ast.elements[0].type).toBe(NodeType.SELECT);
    });
  });
