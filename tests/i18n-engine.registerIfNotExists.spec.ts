import { I18nEngine, createDefaultLanguages, LanguageCodes } from '../src';
...
describe('I18nEngine.registerIfNotExists', () => {
  ...
  describe('static registerIfNotExists', () => {
    it('should create instance if it does not exist', () => {
      const engine = I18nEngine.registerIfNotExists('test', createDefaultLanguages());
    ...
    it('should return existing instance if it already exists', () => {
      const engine1 = I18nEngine.registerIfNotExists('test', createDefaultLanguages());
      const engine2 = I18nEngine.registerIfNotExists('test', createDefaultLanguages());
    ...
    it('should work with default instance key', () => {
      const engine1 = I18nEngine.registerIfNotExists('default', createDefaultLanguages());
      const engine2 = I18nEngine.registerIfNotExists('default', createDefaultLanguages());
  ...
  describe('instance registerIfNotExists', () => {
    ...
    it('should not register component if it already exists', () => {
      ...
      expect(result.isValid).toBe(true);
      expect(engine.translate('test-component', 'test.key')).toBe('Original Value');
    });
  ...
  describe('integration with multiple modules', () => {
    it('should handle multiple modules initializing the same instance', () => {
      ...
      expect(engine1.translate('module1', 'module1.key')).toBe('Module 1 Value');
      expect(engine1.translate('module2', 'module2.key')).toBe('Module 2 Value');
    });
  });
});
