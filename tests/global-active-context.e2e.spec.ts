...
describe('GlobalActiveContext E2E', () => {
  let globalContext: GlobalActiveContext<DefaultLanguageCode, TestActiveContext>;

  beforeEach(() => {
    // Clear static state
    (GlobalActiveContext as any)._contextMap.clear();
    (GlobalActiveContext as any)._instance = undefined;
    globalContext = new GlobalActiveContext<DefaultLanguageCode, TestActiveContext>();
  });
  ...
  describe('real-world usage scenarios', () => {
    ...
    it('should handle multi-tenant application with separate contexts', () => {
      ...
      expect(globalContext.getContext('tenant1').language).toBe(LanguageCodes.EN_US);
      ...
      expect(globalContext.getContext('tenant2').language).toBe(LanguageCodes.FR);
    ...
    it('should handle singleton instance across modules', () => {
      ...
      const instance2 = GlobalActiveContext.getInstance<DefaultLanguageCode, TestActiveContext>();
    ...
    it('should handle error recovery scenarios', () => {
      ...
      expect(() => globalContext.setUserLanguage(LanguageCodes.FR, 'invalid')).toThrow();
  ...
  describe('performance and memory scenarios', () => {
    it('should handle multiple context creation and cleanup', () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        globalContext.createContext(LanguageCodes.EN_US, LanguageCodes.EN_US, key);
      }
      ...
      expect(globalContext.getContext('context_0').language).toBe(LanguageCodes.FR);
      expect(globalContext.getContext('context_50').language).toBe(LanguageCodes.ES);
      expect(globalContext.getContext('context_99').language).toBe(LanguageCodes.EN_US);
    });
