 ...
 */
export function getPluralCategory(language: string, count: number): string {
...
export function hasPluralForm(language: string, category: string): boolean {
  const forms = LANGUAGE_PLURAL_FORMS[language] || LANGUAGE_PLURAL_FORMS['en'];
  return forms.required.includes(category as any) || forms.optional.includes(category as any);
}
